...
export default function DowntimesPage() {
  ...
  const columns: Column<any>[] = [
    ...
    { key: "category", header: "Catégorie", accessor: (r) => {
      ...
      return cat ? <Badge variant="info">{cat.name}</Badge> : "—";
    }},
    { key: "subcat", header: "Sous-catégorie", accessor: (r) => r.subCategory?.name || "—" },
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? <div className="flex h-32 items-center justify-center"><div className="h-6 w-6 animate-spin rounded-full border-4 border-blue-600 border-t-transparent" /></div> : (
        <DataTable data={events} columns={columns} pageSize={15} searchable searchFn={(r, q) => (r.batch?.lot || "").toLowerCase().includes(q) || (r.subCategory?.name || "").toLowerCase().includes(q)} />
      )}
    </div>
  );
}
